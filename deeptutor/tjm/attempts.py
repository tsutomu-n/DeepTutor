...
class AttemptService:
    ...
    def start_attempt(self, *, exam_id: str, mode: str) -> dict[str, Any]:
        if mode not in {"practice", "exam"}:
            raise DomainValidationError(f"unsupported attempt mode: {mode}")
        exam = self.catalog.get_exam(exam_id)
        versions = self.catalog.selected_published_versions(exam_id)
        return self._create_attempt(exam=exam, versions=versions, mode=mode)

    def _create_attempt(
        self, *, exam: dict[str, Any], versions: list[dict[str, Any]], mode: str
    ) -> dict[str, Any]:
    ...
    def start_review_attempt(self, *, exam_id: str, limit: int = 20) -> dict[str, Any]:
        if not 1 <= limit <= 100:
        ...
        for row in rows:
            ...
            if version["exam_id"] == exam_id:
                versions.append(version)
            if len(versions) >= limit:
                break
        if not versions:
            raise InvalidTransitionError("review queue has no pending questions for this exam")
        return self._create_attempt(exam=exam, versions=versions, mode="review")
    ...
    def record_answer(
        ...
        *,
        ...
        with self.learning.connect() as conn:
            ...
            conn.execute(
                ...
                INSERT INTO answer_events (
                    ...
                    elapsed_ms, client_created_at, created_at
                ) VALUES (?, ?, 'selected', ?, ?, ?, ?, ?)
                """,
                (
                    attempt_id,
        ...
        return self.get_attempt(attempt_id)["items"][position]

    def use_hint(self, attempt_id: str, *, position: int, elapsed_ms: int) -> dict[str, Any]:
        ...
        with self.learning.connect() as conn:
            ...
            conn.execute(
                ...
                """,
                (attempt_id, position, elapsed_ms, now),
            )
            conn.execute(
                """
                UPDATE attempt_items SET
                    opened_at = COALESCE(opened_at, ?), hint_count = hint_count + 1
                WHERE attempt_id = ? AND position = ?
                """,
                (now, attempt_id, position),
            )
        return {"hint": hints[hint_index], "hint_number": hint_index + 1}
        ...
        *,
        ...
        with self.learning.connect() as conn:
            ...
            cursor = conn.execute(
                ...
                """,
                (attempt_id, position, proposed, elapsed_ms, recognized, now),
            )
        ...
        *,
        ...
        with self.learning.connect() as conn:
            ...
            conn.execute(
                ...
                INSERT INTO answer_events (
                    attempt_id, position, event_type, option_key, confidence,
                    elapsed_ms, transcript, created_at
            ...
            conn.execute(
                """
                UPDATE attempt_items SET
                    opened_at = COALESCE(opened_at, ?), answered_at = ?,
                    confirmed_option_key = ?, confidence = ?, elapsed_ms = ?
                WHERE attempt_id = ? AND position = ?
                """,
                (now, now, option_key, confidence, elapsed_ms, attempt_id, position),
            )
        return self.get_attempt(attempt_id)["items"][position]
    ...
    def submit_attempt(self, attempt_id: str) -> dict[str, Any]:
        ...
        with self.learning.connect() as conn:
            ...
            expired = attempt["mode"] == "exam" and self._deadline_passed(dict(attempt), now_dt)
            final_status = "expired" if expired else "submitted"
            ...
            conn.execute(
                ...
                WHERE id = ?
                """,
                (final_status, now, grade.correct, grade.total, attempt_id),
            )
        return self.get_attempt(attempt_id)
    ...
    def analytics(self) -> dict[str, Any]:
        ...
        for raw in item_rows:
            ...
            if item["elapsed_ms"] is not None and item_answered:
                elapsed_values.append(int(item["elapsed_ms"]))
    ...
    def _validate_metrics(*, confidence: int | None, elapsed_ms: int) -> None:
        if isinstance(elapsed_ms, bool) or not isinstance(elapsed_ms, int) or elapsed_ms < 0:
            raise DomainValidationError("elapsed_ms must be a non-negative integer")
    ...
    def _ensure_answerable(self, attempt: dict[str, Any]) -> None:
        if attempt["status"] != "in_progress":
            raise AlreadySubmittedError("attempt is already finalized")
        if attempt["mode"] == "exam" and self._deadline_passed(attempt, _now_datetime()):
    ...
    def _enqueue_review_reasons(
        ...
        *,
        ...
        if (
            ...
            and int(item["elapsed_ms"]) >= self.review_policy.slow_correct_ms
        ):
